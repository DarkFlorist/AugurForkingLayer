...
test('rejects sibling and excluded-product imports while allowing local packages', () => {
	...
	expect(checkImportBoundary('/repo/zoltar/ui/app.ts', '@zoltar/unrelated-shared/math', '/repo/zoltar')).toContain('excluded')
	expect(checkImportBoundary('/repo/zoltar/ui/app.ts', '@zoltar/ui-unrelated/x', '/repo/zoltar')).toContain('excluded')

...
if (JSON.stringify(hashes) !== JSON.stringify(baseline)) throw new Error('Production artifacts differ from the pinned upstream baseline. Review ABI, bytecode and deployment-address changes before updating the baseline.')
console.log(`${Object.keys(hashes).length} production contract ABIs and bytecodes match the upstream baseline`)
